...
export default class Trace {
  ...
  update(source, callback) {
    ...
    nodeEnter
      ...
      .text((d) => `${d.data.layer || ''} ${d.data.component ? '- ' + d.data.component : d.data.component || ''}`);
    nodeEnter
      .append('rect')
      .attr('rx', 2)
      .attr('ry', 2)
    ...
    node
      ...
      .style('opacity', 1)
      .select('circle')
      .attr('fill', (d) => (d._children ? `${this.sequentialScale(this.list.indexOf(d.data.serviceCode))}` : ''));
